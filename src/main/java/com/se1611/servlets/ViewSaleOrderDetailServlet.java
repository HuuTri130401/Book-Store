...
import com.se1611.orderdetail.OrderItemObj;
import com.se1611.employees.EmployeeDTO;
import com.se1611.book.BookDAO;
import com.se1611.bookCategory.CategoryDAO;
import com.se1611.bookCategory.CategoryDTO;
/**
 *
 * @author Admin
 */
...
public class ViewSaleOrderDetailServlet extends HttpServlet {
     ...
     */
    private final String DETAIL_LIST_PAGE = "sellerOrderDetails.jsp";
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<CategoryDTO> categoryList = cate.getCateList();

            request.setAttribute("detailList", detailList);
            request.setAttribute("categoryList", categoryList);

        } catch (SQLException ex) {
            log("ViewSaleOrderDetailServlet_ SQLException: " + ex.getMessage());
        } catch (ClassNotFoundException ex) {
            log("ViewSaleOrderDetailServlet_ ClassNotFoundException: " + ex.getMessage());
        } catch (NamingException ex) {
            log("ViewSaleOrderDetailServlet_ NamingException: " + ex.getMessage());
        } finally {
